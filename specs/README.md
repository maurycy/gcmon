...
| [0069](0069-group-every-row-an-interpreter-owns.md) | Bug (reporting) | L | gcmon draws each interpreter as an OS thread that does not exist and hangs the rest of its rows off the process track beside it, where the trace processor merges its per-interpreter `GC Metrics` groups into one and no query can say which interpreter a counter belongs to |
...
| 0069 | ADR-0027 is unbuilt until this lands, and every query written against the current shape is one more to rewrite |
| 0030 | |
