# gc-monitor - zero-overhead GC monitoring for Python

[![PyPI](https://img.shields.io/pypi/v/gc-monitor.svg)](https://pypi.org/project/gc-monitor/)
[![CI](https://github.com/sergey-miryanov/gc-monitor/actions/workflows/ci.yml/badge.svg)](https://github.com/sergey-miryanov/gc-monitor/actions/workflows/ci.yml)
[![Python Version](https://img.shields.io/badge/python-3.15+-blue.svg)](https://pypi.org/project/gc-monitor/)
...
gc-monitor reads GC statistics directly from a target process's memory via
...
| **gc-monitor** | **No** | **Yes** | **Yes** | **Yes — zero in-process cost** |
...
gc-monitor runs **outside** the target process. It reads GC statistics directly
...
For the pyperf hook integration, gc-monitor uses an **external process model**:

1. The hook spawns the `gc-monitor` CLI as a separate process
...
(gc-monitor crashes don't affect the target), and clean separation of concerns.

## Limitations

The monitoring and monitored processes must use the **exact same Python version
and build**. `gc-monitor` reads GC statistics directly from the target process's
...
gc-monitor run -s my_script.py

# Monitor with custom output and statistics output
gc-monitor 12345 -o trace.json --stats
```
...
**Use gc-monitor when you want to:**
...
- Monitor across different Python builds — gc-monitor requires the exact same binary (see [Limitations](#limitations))

## CLI Usage

The `gc-monitor` command uses subcommands (`monitor`, `run`). If no subcommand
...
gc-monitor 12345 --output trace.json --rate 0.01
```
...
**Important:** All options and arguments after `-s`/`--script` or `-m`/`--module` are passed verbatim to the target — they are **not** interpreted by gc-monitor. Place gc-monitor options before the target.
...
gc-monitor run --stats --table-format md -m test test_gc -v

# Pass arguments to the script; everything after -s goes to the target
gc-monitor run -s benchmark.py --iterations 1000 --verbose

# Run a module with GC monitoring options
gc-monitor run --format jsonl -o trace.jsonl --stats -m http.server 8000
```
...
| `-o, --output` | both | Output file path for trace data | `gc_trace.json` (chrome), `gc_monitor.jsonl` (JSONL) |
...
|----------|----------------|-------------|---------|
| `GC_MONITOR_OUTPUT` | `-o, --output` | Output file path for trace data | `gc_trace.json` (chrome), `gc_monitor.jsonl` (JSONL) |
| `GC_MONITOR_RATE` | `-r, --rate` | Polling rate in seconds | `0.1` |
| `GC_MONITOR_DURATION` | `-d, --duration` | Monitoring duration in seconds | Until interrupted / script exits |
| `GC_MONITOR_VERBOSE` | `-v, --verbose` | Verbose level (integer or truthy value) | `0` |
| `GC_MONITOR_FORMAT` | `--format` | Output format: `chrome`, `jsonl`, or `stdout` | `chrome` |
| `GC_MONITOR_FLUSH_THRESHOLD` | `--flush-threshold` | Number of events to buffer before flushing (JSONL format) | `100` |
| `GC_MONITOR_STATS` | `--stats` | Enable statistics table (`1`, `true`, `yes`, `on`) | `False` |
| `GC_MONITOR_TABLE_FORMAT` | `--table-format` | Table format: `plain`, `md`, or `markdown` | `plain` |
...
```bash
$ gc-monitor 12345 --stats --table-format md
...
```bash
export GC_MONITOR_PYPERF_HOOK_OUTPUT="gc_monitor_{bench_name}.jsonl"
# Run benchmark with GC monitoring and JSONL output
python my_benchmark.py --hook=gc_monitor --inherit-environ=GC_MONITOR_PYPERF_HOOK_OUTPUT -p 5
...
it tells pyperf to pass `GC_MONITOR_PYPERF_HOOK_OUTPUT` from the parent shell to
...
|----------|-------------|---------|
| `GC_MONITOR_PYPERF_HOOK_OUTPUT` | Output path for the combined GC trace file (JSONL). Supports `{bench_name}` and `{pid}` substitution. | `gc_monitor_{bench_name}_combined_{pid}.jsonl` |
| `GC_MONITOR_PYPERF_HOOK_TEMP_DIR` | Directory for temporary JSONL files written during monitoring. | System temp directory |
| `GC_MONITOR_PYPERF_HOOK_VERBOSE` | Enable verbose logging from the hook. Accepts `1`, `yes`, `on`, or `true` (case-insensitive). | Disabled |
...
Bug reports and pull requests are welcome at [GitHub](https://github.com/sergey-miryanov/gc-monitor/issues).
